...
const publicImages = {
  ...
  hairFour: assetPath('images/hairs4.jpg'),
  hairFive: assetPath('images/hairs5.jpg'),
  nailsOne: assetPath('images/nails1.jpg'),
  nailsTwo: assetPath('images/nails2.jpg'),
  nailsThree: assetPath('images/nails3.jpg'),
  logo: assetPath('images/logo.png'),
};

function Logo() {
  return <Link href="/" className="group flex items-center gap-3" data-testid="link-logo">
    <img src={publicImages.logo} alt="ZANNI Beauty Salon" className="h-12 w-12 rounded-full object-cover transition-transform duration-300 group-hover:scale-[1.04]" />
  </Link>;
}
...
function Home() {
  return <Shell><main>
    ...
    <section className="mx-auto grid max-w-[1320px] gap-12 px-6 py-20 md:grid-cols-[.7fr_1.3fr] md:gap-28 md:px-10 md:py-32"><div><SectionLabel>The Zanni approach</SectionLabel><div className="relative mt-8 hidden aspect-[4/5] overflow-hidden rounded-t-full bg-muted md:block"><img src={portraitImage} alt="Soft profile portrait" className="image-editorial h-full w-full object-cover" /><div className="logo-stamp absolute bottom-5 right-5 rounded-2xl bg-background/95 p-2 shadow-lg"><img src={publicImages.logo} alt="ZANNI Beauty Salon logo" className="h-16 w-16 rounded-xl object-cover" /></div></div></div><div className="flex flex-col justify-center"><h2 className="max-w-[750px] font-serif text-[clamp(2.6rem,5vw,5.4rem)] leading-[1.01] tracking-[-.05em]">Beauty should feel like a conversation, not a <em className="text-primary">prescription.</em></h2><p className="mt-8 max-w-[580px] text-[15px] leading-7 text-muted-foreground">From a quiet reset to a fresh new look, we keep every detail personal. We listen first, work with care, and make space for the version of you that is already there.</p><p className="mt-8 font-serif text-2xl italic text-primary">#VIP Result Na Hindi Masakit Sa Bulsa</p><Link href="/contact" className="mt-9 inline-flex w-fit items-center gap-3 text-[10px] font-bold uppercase tracking-[.17em] text-primary hover:text-foreground">Start with a hello <ArrowDownRight size={16} /></Link></div></section>
